$(function () {
    $("head").append(
        '<script type="text/javascript">localStorage.setItem("shops", JSON.stringify(shops));</script>'
    );
    $("head").append(
        '<script type="text/javascript">localStorage.setItem("products", JSON.stringify(products));</script>'
    );
    ...
    for (var i = 0; i < shops.length; i++) {
        shopOptions.push('<option value="' + shops[i] + '" />');
    }
    ...
    for (var i = 0; i < products.length; i++) {
        productOptions.push('<option value="' + products[i] + '" />');
    }
